...
def preprocess_text(text, stopwords, stemmer):
    """
    Modified version without stopwords filtering
    """
    tokens = text.lower()
    pattern = re.compile(r'[a-z]+')
    tokens = pattern.findall(tokens)
    # COMMENTED OUT: stopwords filtering
...
class BooleanSearchEngine:
    """
    Boolean search engine using TF-IDF scoring
    """
...
if __name__ == "__main__":
    print("=== No Stopwords Challenge Test ===")
    print("Testing impact of removing stopwords filtering...")
    
    # Load data
    stopwords_path = 'stopwords.txt'
    stopwords = load_stopwords(stopwords_path)
    
    # Force rebuild index (ignore existing index)
    print("Rebuilding index without stopwords filtering...")
    ...
    print(f"Index building time: {index_time:.2f}s")
    
    # Save the new index
    print_inverted_index(inverted_index)
    print(f"Index saved with {len(inverted_index)} terms")
    
    # Boolean retrieval
    print("Processing Boolean queries...")
    ...
    print(f"Boolean query processing time: {boolean_time:.2f}s")
    
    # Ranked retrieval
    print("Processing Ranked queries...")
    ranked_start = time.time()
    ...
    ranked_time = time.time() - ranked_start
    print(f"Ranked query processing time: {ranked_time:.2f}s")
